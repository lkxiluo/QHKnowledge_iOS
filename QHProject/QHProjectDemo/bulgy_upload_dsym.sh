...
#
# #
BUGLY_APP_ID="2b0e5f5255"
# #
BUGLY_APP_KEY="75dc2133-c0b2-4b28-87e2-d23a996ec5cc"
# #
BUNDLE_IDENTIFIER="com.gree.QHProjectDemo"
# #
UPLOAD_DSYM_ONLY=1
#
...
# # Test模式编译是否上传，1＝上传 0＝不上传，默认不上传
UPLOAD_TEST_SYMBOLS=0

# # DEMO模式编译是否上传，1＝上传 0＝不上传，默认不上传
UPLOAD_DEMO_SYMBOLS=0
#
    ...
    P_BSYMBOL_ZIP_FILE_NAME=${P_BSYMBOL_ZIP_FILE##*/}
        ...
        function run() {
            ...
            function runInXcode(){
                ...
                if [ $UPLOAD_TEST_SYMBOLS -eq 0 ]; then
                exitWithMessage "Warning: Build for test mode and skipping to upload. \nYou can modify 'UPLOAD_TEST_SYMBOLS' to 1 in the script." 0
                fi
                fi

                if [ "${CONFIGURATION=}" == "Demo" ]; then
                if [ $UPLOAD_DEMO_SYMBOLS -eq 0 ]; then
                exitWithMessage "Warning: Build for demo mode and skipping to upload. \nYou can modify 'UPLOAD_DEMO_SYMBOLS' to 1 in the script." 0
                fi
                fi
